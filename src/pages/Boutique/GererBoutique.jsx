...
export default function GererBoutique() {
  ...
  const partagerLienBoutique = async () => {
    if (navigator.share) {
      try {
        await navigator.share({ title: boutique.nom, text: `Découvre "${boutique.nom}" sur ${marque.nom} !`, url: lienBoutique });
      } catch {
        // Partage annulé
      }
    } else {
  ...
  const partagerProduit = (p) => {
    const texte = `Regarde ce produit sur ${marque.nom} (${boutique.nom}) : ${p.nom} — ${fmt(p.prix, p.devise)}`;
      ...
      <div className="bg-white rounded-xl p-3.5 mb-3 flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-[11px] text-gray-400 mt-0.5">
            {logoEnvoiEnCours ? "Envoi en cours..." : "Elle apparaît partout où votre boutique est affichée sur {marque.nom}."}
          </p>
      ...
      <div className="bg-white rounded-xl p-3 mb-3">
        ...
        {voirAideLien && (
          <div className="bg-[#FFF8F2] border border-[#FFD3AC] rounded-xl p-3 mb-2 text-[11.5px] text-gray-600 leading-relaxed">
            C'est l'adresse unique de votre boutique sur {marque.nom}. Partagez-la sur WhatsApp, dans vos statuts,
            ...
            {produitEnEdition === p.id ? (
              ...
              <>
                ...
                <div className="p-2.5">
                  ...
                  <button
                    onClick={() => partagerProduit(p)}
                    className="w-full mt-1.5 flex items-center justify-center gap-1 bg-[#25D366] text-white text-[11px] font-semibold rounded-md py-1.5"
                  >
                    <MessageCircle size={12} /> Partager sur WhatsApp
                  </button>
                </div>
              </>
            )}
